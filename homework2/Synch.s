...
_StringConst_5:
	.word	53			! length
	.ascii	"Current thread asking to unlock doesn\'t hold the lock"
	.align
_StringConst_4:
	.word	73			! length
	.ascii	"Current thread asking to lock the mutex already holds a lock on the mutex"
	.align
_StringConst_3:
	.word	51			! length
...
! IF STATEMENT...
	...
	mov	"\0\0IF",r10
!   if _P_Thread_currentThread == heldBy then goto _Label_67		(int)
	set	_P_Thread_currentThread,r1
	load	[r1],r1
	load	[r14+8],r2
	load	[r2+16],r2
...
! IF STATEMENT...
	...
	mov	"\0\0IF",r10
!   if t == 0 then goto _Label_71		(int)
	load	[r14+-32],r1
	mov	0,r2
	cmp	r1,r2
	be	_Label_71
!	_Label_70	jmp	_Label_70
_Label_70:
! THEN...
...
!   _temp_72 = t + 76
	...
	add	r1,76,r1
	store	r1,[r14+-16]
!   Data Move: *_temp_72 = 2  (sizeInBytes=4)
	mov	2,r1
	load	[r14+-16],r2
	store	r1,[r2]
! SEND STATEMENT...
...
! RETURN STATEMENT...
	...
	mov	"\0\0RE",r10
!   if _P_Thread_currentThread != heldBy then goto _Label_84		(int)
	set	_P_Thread_currentThread,r1
	load	[r1],r1
	load	[r14+8],r2
	load	[r2+16],r2
